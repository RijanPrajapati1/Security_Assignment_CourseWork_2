...
//                                             <td className="p-3">{payment.transactionId || "37t42432479832842"}</td>
//                                             <td className="p-3">
//                                                 <span
...
const maskTransactionId = (id) => {
    if (!id || id.length <= 4) {
        return id; // If too short or null, return as is
    }
    // Show only the last 4 characters and mask the rest with asterisks
    return '************' + id.slice(-4);
};
                                ...
                                <tbody>
                                    {payments.map((payment, index) => (
                                        <tr key={payment._id} className="border-b hover:bg-gray-200">
                                            ...
                                            <td className="p-3">{payment.paymentMethod}</td>
                                            <td className="p-3">
                                                {/* Apply the maskTransactionId function here */}
                                                {maskTransactionId(payment.transactionId || "N/A")}
                                            </td>
                                            <td className="p-3">
                                                <span
                                                    className={`px-3 py-1 rounded-full text-xs font-semibold ${payment.paymentStatus === "pending"
